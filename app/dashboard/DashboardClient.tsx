...
function InicioTab({
  ...
  const loadData = async () => {
    ...
    // Load ranking via API server-side (usa service role, bypasea RLS, misma lógica que RankingTab)
    const [allProfilesRes, rankingRes] = await Promise.all([
      supabase.from('profiles').select('*'),
      fetch('/api/ranking')
    ])
    const allProfilesData = allProfilesRes.data
    const rankingData: Array<{ id: string; display_name: string; total_points: number }> =
      rankingRes.ok ? await rankingRes.json() : []
    ...
    if (allProfilesData && rankingData.length > 0) {
      // Merge profile data with computed totals from API
      const totalsByUser: Record<string, number> = {}
      rankingData.forEach(r => { totalsByUser[r.id] = r.total_points })

      const withTotals = allProfilesData.map((p: any) => ({
        ...p,
        total_points: totalsByUser[p.id] ?? 0,
      }))
...
function RankingTab({ currentUserId }: { currentUserId: string }) {
  ...
  async function loadRanking() {
    ...
    // Fetch group bonuses from server API (bypasses RLS on group_position_predictions)
    const rankingApiRes = await fetch('/api/ranking')
    const rankingApiData: Array<{ id: string; group_bonus: number }> = rankingApiRes.ok
      ? await rankingApiRes.json()
      : []
    const groupBonusByUser: Record<string, number> = {}
    rankingApiData.forEach(r => { groupBonusByUser[r.id] = r.group_bonus || 0 })
    ...
    const profilesWithBreakdown = await Promise.all(
      profilesData.map(async (profile) => {
        ...
        // Group order bonus — sourced from /api/ranking which uses service role (bypasses RLS)
        groupOrderBonus = groupBonusByUser[profile.id] || 0
